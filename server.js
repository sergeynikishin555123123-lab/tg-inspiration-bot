    ...
    (err, existingCompletion) => {
      ...
      db.get("SELECT * FROM quizzes WHERE id = ?", [quizId], (err, quiz) => {
        ...
        if (correctAnswers >= passThreshold) {
          // Базовая награда за прохождение
          sparksEarned = quiz.sparks_reward;
          
          // Бонус за все правильные ответы
          if (correctAnswers === questions.length) {
            sparksEarned += 4; // +4 за идеальное прохождение (итого 5)
          } else {
        ...
        res.json({
          ...
          perfectScore: correctAnswers === questions.length,
          message: sparksEarned > 0 ? 
            `Поздравляем! Вы получили ${sparksEarned}✨` + 
            (correctAnswers === questions.length ? ' (идеальный результат!)' : '') 
            : 'Попробуйте еще раз!'
        });
      });
    }
  ...
  db.get('SELECT * FROM invitations WHERE inviter_id = ? AND invited_id = ?', [inviterId, invitedId], (err, existingInvite) => {
    ...
    db.run(`INSERT INTO invitations (inviter_id, invited_id, invited_username) VALUES (?, ?, ?)`,
      ...
      function(err) {
        ...
        // Начисляем бонус пригласившему - 10 искр по новой системе
...
// Получение постов канала для комментариев
app.get('/api/webapp/posts', (req, res) => {
  db.all("SELECT id, post_id, title FROM channel_posts WHERE is_published = TRUE ORDER BY published_at DESC LIMIT 20", 
    (err, posts) => {
      if (err) return res.status(500).json({ error: 'Database error' });
      res.json(posts);
    }
...
// Отправка комментария (отзыва) к посту
app.post('/api/webapp/comments', (req, res) => {
  const { userId, postId, commentText } = req.body;
  
  console.log('💬 Отправка комментария от пользователя:', userId, 'к посту:', postId);
  
  if (!userId || !commentText || !postId) {
  ...
  db.get(`SELECT * FROM comments WHERE user_id = ? AND post_id = ? AND DATE(created_at) = DATE('now')`, 
    ...
    if (todayComment) {
      return res.json({
        success: true,
        message: 'Вы уже оставляли комментарий к этому посту сегодня',
        sparksAwarded: 0
      });
    }
    ...
    db.run(`INSERT INTO comments (user_id, post_id, comment_text) VALUES (?, ?, ?)`,
      ...
      function(err) {
        ...
        // Начисляем 1 искру сразу за комментарий
        const sparksAwarded = 1;
        db.run(`UPDATE users SET sparks = sparks + ? WHERE user_id = ?`, [sparksAwarded, userId]);
        db.run(`UPDATE comments SET is_approved = TRUE, sparks_awarded = TRUE WHERE id = ?`, [this.lastID]);
        db.run(`INSERT INTO activities (user_id, activity_type, sparks_earned, description) VALUES (?, 'comment', ?, 'Комментарий к посту')`,
        ...
        db.get('SELECT * FROM purchases WHERE user_id = ? AND item_id = ?', [userId, itemId], (err, existingPurchase) => {
          ...
          db.run('UPDATE users SET sparks = sparks - ? WHERE user_id = ?', [item.price, userId], function(err) {
            ...
            db.run('INSERT INTO purchases (user_id, item_id, price_paid) VALUES (?, ?, ?)', 
              ...
              res.json({
                success: true,
                message: 'Покупка успешно завершена! Товар добавлен в вашу коллекцию.',
                item: item,
...
app.post('/api/admin/posts', requireAdmin, (req, res) => {
  ...
  db.run(`INSERT INTO channel_posts (title, content, photo_url, video_url, buttons, requires_action, action_type, action_target, published_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    function(err) {
      ...
      // Автоматически публикуем пост в канал
      db.get('SELECT * FROM channel_posts WHERE id = ?', [this.lastID], (err, post) => {
        if (!err && post) {
...
app.get('/api/admin/comments', requireAdmin, (req, res) => {
  db.all(`
    ...
    JOIN channel_posts cp ON c.post_id = cp.post_id
...
app.post('/api/admin/comments/:id/approve', requireAdmin, (req, res) => {
  ...
  db.get('SELECT * FROM comments WHERE id = ?', [id], (err, comment) => {
    ...
    db.run(`UPDATE comments SET is_approved = TRUE WHERE id = ?`, [id]);
    ...
    db.get('SELECT * FROM users WHERE user_id = ?', [inviteCode], (err, inviter) => {
      if (!err && inviter) {
        db.run(`INSERT OR IGNORE INTO invitations (inviter_id, invited_id, invited_username) VALUES (?, ?, ?)`,
          ...
          function() {
            if (this.changes > 0) {
              // Начисляем 10 искр за приглашение
              db.run(`UPDATE users SET sparks = sparks + 10, invite_count = invite_count + 1 WHERE user_id = ?`, [inviteCode]);
              db.run(`INSERT INTO activities (user_id, activity_type, sparks_earned, description) VALUES (?, 'invitation', 10, 'Приглашение друга')`, [inviteCode]);
...
async function publishToChannel(post) {
  try {
    ...
    // Добавляем кнопку "Написать отзыв" для комментариев
    const appUrl = process.env.APP_URL || 'http://localhost:3000';
    keyboard.inline_keyboard.push([{
      text: "💬 Написать отзыв",
      web_app: { url: `${appUrl}#comment?postId=${post.post_id || post.id}` }
    }]);
