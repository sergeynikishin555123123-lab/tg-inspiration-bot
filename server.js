...
app.get('/api/webapp/invite/:userId', (req, res) => {
  const userId = req.params.userId;
  const channelUsername = process.env.CHANNEL_USERNAME || 'inspiration_channel';
...
app.post('/api/admin/shop/items', requireAdmin, (req, res) => {
  ...
  db.run(`INSERT INTO shop_items (title, description, type, file_url, preview_url, price) VALUES (?, ?, ?, ?, ?, ?)`,
    [title, description, type || 'video', file_url, preview_url, price],
    function(err) {
      if (err) {
...
Нажмите кнопку ниже чтобы начать!`;
  ...
  console.log('⚠️ BOT_TOKEN не установлен');
}
